...
#define LOGV(...) \
    (__android_log_print(ANDROID_LOG_VERBOSE, "NanoVG Demo", __VA_ARGS__))
#define LOGI(...) \
    (__android_log_print(ANDROID_LOG_INFO, "NanoVG Demo", __VA_ARGS__))
#define LOGW(...) \
    (__android_log_print(ANDROID_LOG_WARN, "NanoVG Demo", __VA_ARGS__))
#define LOGE(...) \
    (__android_log_print(ANDROID_LOG_ERROR, "NanoVG Demo", __VA_ARGS__))
#define LOGF(...) \
    (__android_log_print(ANDROID_LOG_FATAL, "NanoVG Demo", __VA_ARGS__))
...
FILE *android_fopen(const char *fname, const char *mode)
{
    ...
    if (strchr(mode, 'w')) { // for writing
        LOGE("Unable to open asset (trying to write): '%s' ('%s')", fname,
             mode);
        return NULL;
    }

    if (!android_asset_manager) {
        LOGE("Unable to open asset (unknown asset manager): '%s' ('%s')", fname,
             mode);
        return NULL;
    }
