...
{
    bndSetFont(nvgCreateFont(vg, "system",
...
void drawUI(NVGcontext *vg, int item, int corners)
{
    ...
    if (head) {
        ...
        case ST_BUTTON: {
            const UIButtonData *data = (UIButtonData *)head;
            bndToolButton(vg, rect.x, rect.y, rect.w, rect.h, corners,
        ...
        case ST_RADIO: {
            ...
            if (*data->value == item)
                state = BND_ACTIVE;
            bndRadioButton(vg, rect.x, rect.y, rect.w, rect.h, corners, state,
        ...
        case ST_SLIDER: {
            ...
            bndSlider(vg, rect.x, rect.y, rect.w, rect.h, corners, state, *data->progress,
                      data->label, value);
        } break;
        case ST_TEXT: {
            ...
            bndTextField(vg, rect.x, rect.y, rect.w, rect.h, corners, state, -1,
                         data->text, idx, idx);
        } break;
        case ST_DEMOSTUFF: {
        ...
        case ST_RECT: {
            ...
            if (rect.w && rect.h) {
                ...
                nvgSave(vg);
                nvgStrokeColor(
                    vg, nvgRGBAf(data->color.r, data->color.g, data->color.b, 0.9f));
                if (state != BND_DEFAULT) {
                    nvgFillColor(
                        vg, nvgRGBAf(data->color.r, data->color.g, data->color.b, 0.5f));
                } else {
                    nvgFillColor(
                        vg, nvgRGBAf(data->color.r, data->color.g, data->color.b, 0.1f));
                }
                nvgStrokeWidth(vg, 2);
                nvgBeginPath(vg);
#if 0
...
void draw_noodles(NVGcontext *vg, int x, int y)
{
    ...
    for (int i = 0; i < 9; ++i) {
        int a = i % 3;
        int b = i / 3;
        bndNodeWire(vg, x, y + s * a, x + w, y + s * b, (BNDwidgetState)a,
                    (BNDwidgetState)b);
    }
...
void draw_demostuff(NVGcontext *vg, int x, int y, float w, float h)
{
    ...
    y += 40;
    bndRadioButton(vg, x, y, 80, BND_WIDGET_HEIGHT, BND_CORNER_NONE, BND_DEFAULT, -1,
                   "Default");
    y += 25;
    bndRadioButton(vg, x, y, 80, BND_WIDGET_HEIGHT, BND_CORNER_NONE, BND_HOVER, -1,
                   "Hovered");
    y += 25;
    bndRadioButton(vg, x, y, 80, BND_WIDGET_HEIGHT, BND_CORNER_NONE, BND_ACTIVE, -1,
                   "Active");

    y += 25;
    bndLabel(vg, x, y, 120, BND_WIDGET_HEIGHT, -1, "Label:");
    y += BND_WIDGET_HEIGHT;
    bndChoiceButton(vg, x, y, 80, BND_WIDGET_HEIGHT, BND_CORNER_NONE, BND_DEFAULT, -1,
                    "Default");
    y += 25;
    bndChoiceButton(vg, x, y, 80, BND_WIDGET_HEIGHT, BND_CORNER_NONE, BND_HOVER, -1,
                    "Hovered");
    y += 25;
    bndChoiceButton(vg, x, y, 80, BND_WIDGET_HEIGHT, BND_CORNER_NONE, BND_ACTIVE, -1,
                    "Active");
    ...
    y += 40;
    bndNumberField(vg, x, y, 120, BND_WIDGET_HEIGHT, BND_CORNER_DOWN, BND_DEFAULT, "Top",
                   "100");
    ...
    bndMenuItem(vg, mx, my, mw, BND_WIDGET_HEIGHT, BND_DEFAULT, BND_ICONID(17, 3),
                "Default");
    my += BND_WIDGET_HEIGHT - 2;
    bndMenuItem(vg, mx, my, mw, BND_WIDGET_HEIGHT, BND_HOVER, BND_ICONID(18, 3),
                "Hovered");
    my += BND_WIDGET_HEIGHT - 2;
    bndMenuItem(vg, mx, my, mw, BND_WIDGET_HEIGHT, BND_ACTIVE, BND_ICONID(19, 3),
                "Active");
    ...
    y += 40;
    bndRadioButton(vg, x, y, 60, BND_WIDGET_HEIGHT, BND_CORNER_RIGHT, BND_DEFAULT, -1,
                   "One");
    x += 60 - 1;
    bndRadioButton(vg, x, y, 60, BND_WIDGET_HEIGHT, BND_CORNER_ALL, BND_DEFAULT, -1,
                   "Two");
    x += 60 - 1;
    bndRadioButton(vg, x, y, 60, BND_WIDGET_HEIGHT, BND_CORNER_ALL, BND_DEFAULT, -1,
                   "Three");
    x += 60 - 1;
    bndRadioButton(vg, x, y, 60, BND_WIDGET_HEIGHT, BND_CORNER_LEFT, BND_ACTIVE, -1,
                   "Butts");
    ...
    ry += 25;
    bndTextField(vg, rx, ry, 240, BND_WIDGET_HEIGHT, BND_CORNER_NONE, BND_DEFAULT, -1,
                 edit_text, idx1, idx2);
    ry += 25;
    bndTextField(vg, rx, ry, 240, BND_WIDGET_HEIGHT, BND_CORNER_NONE, BND_HOVER, -1,
                 edit_text, idx1, idx2);
    ry += 25;
    bndTextField(vg, rx, ry, 240, BND_WIDGET_HEIGHT, BND_CORNER_NONE, BND_ACTIVE, -1,
                 edit_text, idx1, idx2);
...
void build_democontent(int parent)
{
...

    {
        ...
        if (option2)
            uiSetMargins(hgroup_append_fixed(h, radio(BND_ICON_REC, NULL, &enum1)), -1, 0,
                         0, 0);
        uiSetMargins(hgroup_append_fixed(h, radio(BND_ICON_PLAY, NULL, &enum1)), -1, 0, 0,
                     0);
        uiSetMargins(hgroup_append(h, radio(BND_ICON_GHOST, "Item 3.3", &enum1)), -1, 0,
                     0, 0);
    }

    {
        ...
        uiSetMargins(
            vgroup_append(coll, button(BND_ICON_GHOST, "Item 4.0.1", demohandler)), 0, -2,
            0, 0);
...
int demorect(int parent, const char *label, float hue, int box, int layout, int w, int h,
...
void build_layoutdemo(int parent)
{
    ...
    int box = demorect(parent, "Box( UI_LAYOUT )\nLayout( UI_FILL )", 0.6f, UI_LAYOUT,
                       UI_FILL, 0, 0, M, M, M, M);
    demorect(box, "Layout( UI_HFILL | UI_TOP )", 0.7f, 0, UI_HFILL | UI_TOP, S, S + M, M,
             M, M, 0);
    demorect(box, "Layout( UI_HFILL )", 0.7f, 0, UI_HFILL, S, S + 2 * M, M, 0, M, 0);
    demorect(box, "Layout( UI_HFILL | UI_DOWN )", 0.7f, 0, UI_HFILL | UI_DOWN, S, S + M,
             M, 0, M, M);

    demorect(box, "Layout( UI_LEFT | UI_VFILL )", 0.7f, 0, UI_LEFT | UI_VFILL, S + M, S,
             M, M, 0, M);
    demorect(box, "Layout( UI_VFILL )", 0.7f, 0, UI_VFILL, S + 2 * M, S, 0, M, 0, M);
    demorect(box, "Layout( UI_RIGHT | UI_VFILL )", 0.7f, 0, UI_RIGHT | UI_VFILL, S + M, S,
             0, M, M, M);

    demorect(box, "Layout( UI_LEFT | UI_TOP )", 0.55f, 0, UI_LEFT | UI_TOP, S, S, M, M, 0,
             0);
    demorect(box, "Layout( UI_TOP )", 0.57f, 0, UI_TOP, S, S, 0, M, 0, 0);
    demorect(box, "Layout( UI_RIGHT | UI_TOP )", 0.55f, 0, UI_RIGHT | UI_TOP, S, S, 0, M,
             M, 0);
    ...
    demorect(box, "Layout( UI_LEFT | UI_DOWN )", 0.55f, 0, UI_LEFT | UI_DOWN, S, S, M, 0,
             0, M);
    demorect(box, "Layout( UI_DOWN)", 0.57f, 0, UI_DOWN, S, S, 0, 0, 0, M);
    demorect(box, "Layout( UI_RIGHT | UI_DOWN )", 0.55f, 0, UI_RIGHT | UI_DOWN, S, S, 0,
             0, M, M);
}
...
void build_rowdemo(int parent)
{
    ...
    {
        int box = demorect(parent, "Box( UI_ROW )\nLayout( UI_LEFT | UI_VFILL )", 0.6f,
                           UI_ROW, UI_LEFT | UI_VFILL, 0, S, M, M, M, M);
    ...
    }
    {
        int box = demorect(parent, "Box( UI_ROW | UI_JUSTIFY )\nLayout( UI_FILL )", 0.6f,
                           UI_ROW | UI_JUSTIFY, UI_FILL, 0, S, M, 0, M, M);
...
void build_columndemo(int parent)
{
    ...
    {
        int box = demorect(parent, "Box( UI_COLUMN )\nLayout( UI_TOP | UI_HFILL )", 0.6f,
                           UI_COLUMN, UI_TOP | UI_HFILL, S, 0, M, M, M, M);
...
void fill_wrap_row_box(int box)
{
    ...
    for (int i = 0; i < 20; ++i) {
        ...
        case 1: {
            u = demorect(box, "Layout( UI_VCENTER )", hue, 0, UI_VCENTER, width, T / 2, M,
                         M, M, M);
        } break;
        case 2: {
            u = demorect(box, "Layout( UI_VFILL )", hue, 0, UI_VFILL, width, T, M, M, M,
                         M);
        } break;
        case 3: {
            u = demorect(box, "Layout( UI_DOWN )", hue, 0, UI_DOWN, width, T / 2, M, M, M,
                         M);
        } break;
        }
...
void fill_wrap_column_box(int box)
{
    ...
    for (int i = 0; i < 20; ++i) {
        ...
        default:
            break;
        case 0: {
            u = demorect(box, "Layout( UI_LEFT )", hue, 0, UI_LEFT, T, height, M, M, M,
                         M);
        } break;
        case 1: {
            u = demorect(box, "Layout( UI_HCENTER )", hue, 0, UI_HCENTER, T / 2, height,
                         M, M, M, M);
        } break;
        case 2: {
            u = demorect(box, "Layout( UI_HFILL )", hue, 0, UI_HFILL, T, height, M, M, M,
                         M);
        } break;
        case 3: {
            u = demorect(box, "Layout( UI_RIGHT )", hue, 0, UI_RIGHT, T / 2, height, M, M,
                         M, M);
        } break;
        }
...
void build_wrapdemo(int parent)
{
    ...
    box =
        demorect(col, "Box( UI_ROW | UI_WRAP | UI_MIDDLE )\nLayout( UI_HFILL | UI_TOP )",
    ...
    box = demorect(
        col, "Box( UI_ROW | UI_WRAP | UI_JUSTIFY )\nLayout( UI_HFILL | UI_TOP )", 0.6f,
        UI_ROW | UI_WRAP | UI_JUSTIFY, UI_HFILL | UI_TOP, 0, 0, M, M, M, M);
    fill_wrap_row_box(box);

    box = demorect(
        col, "Box( UI_COLUMN | UI_WRAP | UI_START )\nLayout( UI_LEFT | UI_VFILL )", 0.6f,
        UI_COLUMN | UI_WRAP | UI_START, UI_LEFT | UI_VFILL, 0, 0, M, M, M, M);
...
int main()
{
    ...
    printf("Peak item count: %i (%lu bytes)\nPeak allocated handles: %u bytes\n",
           peak_items, peak_items * (long unsigned int)sizeof(UIitem), peak_alloc);
