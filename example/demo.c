...
void drawEditBoxNum(NVGcontext *vg, const char *text, const char *units, float x, float y,
                    float w, float h)
{
    float uw;
...
void drawButton(NVGcontext *vg, int preicon, const char *text, float x, float y, float w,
                float h, NVGcolor col)
{
    NVGpaint bg;
    char icon[8];
    float cornerRadius = 4.0f;
...
void drawSlider(NVGcontext *vg, float pos, float x, float y, float w, float h)
{
    ...
    // Knob
    knob = nvgLinearGradient(vg, x, cy - kr, x, cy + kr, nvgRGBA(255, 255, 255, 16),
                             nvgRGBA(0, 0, 0, 16));
    nvgBeginPath(vg);
...
void drawEyes(NVGcontext *vg, float x, float y, float w, float h, float mx, float my,
              float t)
{
    NVGpaint gloss, bg;
    float ex = w * 0.23f;
    ...
    gloss = nvgRadialGradient(vg, rx - ex * 0.25f, ry - ey * 0.5f, ex * 0.1f, ex * 0.75f,
                              nvgRGBA(255, 255, 255, 128), nvgRGBA(255, 255, 255, 0));
    nvgBeginPath(vg);
    nvgEllipse(vg, rx, ry, ex, ey);
    nvgFillPaint(vg, gloss);
...
void drawSpinner(NVGcontext *vg, float cx, float cy, float r, float t)
{
    ...
    paint =
        nvgLinearGradient(vg, ax, ay, bx, by, nvgRGBA(0, 0, 0, 0), nvgRGBA(0, 0, 0, 128));
...
void drawThumbnails(NVGcontext *vg, float x, float y, float w, float h, const int *images,
                    int nimages, float t)
{
    float cornerRadius = 3.0f;
    ...
    for (i = 0; i < nimages; i++) {
        ...
        imgPaint = nvgImagePattern(vg, tx + ix, ty + iy, iw, ih, 0.0f / 180.0f * NVG_PI,
                                   images[i], a);
        nvgBeginPath(vg);
    ...
    fadePaint = nvgLinearGradient(vg, x, y + h, x, y + h - 6, nvgRGBA(200, 200, 200, 255),
                                  nvgRGBA(200, 200, 200, 0));
    nvgBeginPath(vg);
    ...
    shadowPaint =
        nvgBoxGradient(vg, x + w - 12 - 1, y + 4 + (h - 8 - scrollh) * u - 1, 8, scrollh,
...
void drawColorwheel(NVGcontext *vg, float x, float y, float w, float h, float t)
{
    ...
    paint =
        nvgRadialGradient(vg, ax, ay, 7, 9, nvgRGBA(0, 0, 0, 64), nvgRGBA(0, 0, 0, 0));
    nvgBeginPath(vg);
...
void drawParagraph(NVGcontext *vg, float x, float y, float width, float height, float mx,
                   float my)
{
    NVGtextRow rows[3];
    ...
    const char *text =
        ...
        "she    was busy jumping over the lazy dog with the fox and all the "
        "men who came to the aid of the party.🎉";
    const char *start;
    const char *end;
    ...
    if (gutter) {
        ...
        nvgRoundedRect(vg, (int)bounds[0] - 4, (int)bounds[1] - 2,
                       (int)(bounds[2] - bounds[0]) + 8, (int)(bounds[3] - bounds[1]) + 4,
...
void renderDemo(NVGcontext *vg, float mx, float my, float width, float height, float t,
                int blowup, DemoData *data)
{
    float x, y, popy;
