...
#define glGetQueryObjectiv(id, pname, params) \
    glGetQueryObjectuiv((id), (pname), (unsigned int *)(params))
#endif
...
void initGPUTimer(GPUtimer *timer)
{
    ...
    (pfnGLGETQUERYOBJECTUI64V)glfwGetProcAddress("glGetQueryObjectui64v");
    ...
    while (available && timer->ret <= timer->cur) {
        ...
        if (available) {
            /*			GLuint64 timeElapsed = 0;
                                    glGetQueryObjectui64v(timer->queries[timer->ret
               % GPU_QUERY_COUNT], GL_QUERY_RESULT, &timeElapsed); timer->ret++;
                                    if (n < maxTimes) {
                                            times[n] =
               (float)((double)timeElapsed * 1e-9); n++;
                                    }*/
        }
    }
    return n;
...
void renderGraph(NVGcontext *vg, float x, float y, PerfGraph *fps)
{
    ...
    if (fps->style == GRAPH_RENDER_FPS) {
        for (i = 0; i < GRAPH_HISTORY_COUNT; i++) {
            float v =
                1.0f / (0.00001f + fps->values[(fps->head + i) % GRAPH_HISTORY_COUNT]);
            float vx, vy;
            if (v > 80.0f)
                v = 80.0f;
